...
// Updated to support hex entities (e.g. &#x41;)
...
export function decodeHtmlEntities(text: string): string {
  ...
  return text.replace(ENTITY_REGEX, (match) => {
    ...
    if (NUMERIC_ENTITY_REGEX.test(match)) {
      ...
      return String.fromCodePoint(Number.parseInt(content, radix));
    }
...
/**
 * Safely stringifies a JSON object for use in a JSON-LD script tag.
 * Escapes HTML entities to prevent XSS attacks (e.g., </script> injection).
 *
