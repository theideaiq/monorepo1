...
export default function PlusBrowsePage() {
  ...
  const fetchCatalog = async () => {
    ...
    const { data, error } = await supabase.from('rental_catalog').select('*').order('id');
    ...
    <div className="min-h-screen bg-[#141414] text-white pb-20 font-sans">
      ...
      <section className="relative h-[70vh] w-full flex items-center justify-start px-4 md:px-12 overflow-hidden">
        ...
        <div className="relative z-10 max-w-2xl mt-20">
          ...
          <Badge variant="brand" className="mb-4 bg-brand-pink text-white border-none">IDEA PLUS ORIGINAL</Badge>
          <h1 className="text-5xl md:text-7xl font-black mb-6 tracking-tighter">
            Unlimited <br/><span className="text-brand-yellow">Possibilities.</span>
          </h1>
          <p className="text-lg text-slate-300 mb-8 leading-relaxed max-w-lg">
            Rent the latest tech, games, and books. Delivered to your doorstep in Baghdad within 2 hours.
          </p>
          <div className="flex gap-4">
             ...
             <Button variant="outline" className="h-14 px-8 text-lg font-bold border-slate-500 text-white hover:bg-white/10">
      ...
      <div className="space-y-12 pl-4 md:pl-12 -mt-20 relative z-20">
        <CategoryRow title="Next-Gen Gaming" items={games} onRent={handleRent} rentingId={rentingId} icon={<PlayCircle size={20} className="text-brand-pink" />} />
        <CategoryRow title="Curated Library" items={books} onRent={handleRent} rentingId={rentingId} icon={<BookOpen size={20} className="text-blue-500" />} />
        <CategoryRow title="Tech & Gear" items={tech} onRent={handleRent} rentingId={rentingId} icon={<Camera size={20} className="text-brand-yellow" />} />
      </div>
