...
import { BookOpen, Check, Gamepad2, Zap, Crown, Film, Music } from 'lucide-react';
...
export default function PlusHome() {
  ...
  const tiers: Tier[] = [
    ...
    },
    {
      ...
      accent: 'border-brand-yellow shadow-[0_0_40px_-10px_rgba(250,204,21,0.3)]',
        ...
        <div className="relative z-10 text-center max-w-4xl px-4">
          ...
          >
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/5 border border-white/10 backdrop-blur-md mb-8">
               ...
               <span className="text-xs font-bold text-white tracking-widest uppercase">The Ultimate Membership</span>
            </div>
            ...
            <p className="text-lg md:text-2xl text-slate-400 mb-10 font-light max-w-xl mx-auto leading-relaxed">
              One subscription. Endless entertainment. Books, Games, and Movies delivered to your door.
            </p>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
               ...
               <Button variant="outline" className="h-14 px-10 rounded-full text-white border-white/20 hover:bg-white/10">
                 Learn More
               </Button>
            </div>
          </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {tiers.map((tier, i) => (
            ...
            >
              ...
              <div className="mb-8">
                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-6 ${tier.popular ? 'bg-brand-yellow text-brand-dark' : 'bg-white/10 text-white'}`}>
                   {tier.icon}
                </div>
                <h3 className="text-2xl font-bold text-white mb-2">{tier.name}</h3>
                <div className="flex items-baseline gap-1">
                  <span className="text-4xl font-black text-white">{tier.price}</span>
                  <span className="text-sm text-slate-500 font-medium">IQD / mo</span>
                </div>
              </div>
              ...
              <ul className="flex-1 space-y-4 mb-8">
                {tier.features.map((feature, f) => (
                  <li key={f} className="flex items-start gap-3 text-sm text-slate-300">
                    <div className={`mt-0.5 w-5 h-5 rounded-full flex items-center justify-center shrink-0 ${tier.popular ? 'bg-brand-yellow/20 text-brand-yellow' : 'bg-white/10 text-slate-500'}`}>
      ...
      <section className="py-24 border-t border-white/5 bg-black/40">
         <div className="max-w-7xl mx-auto px-4">
            <div className="text-center mb-16">
               <h2 className="text-3xl md:text-5xl font-black text-white mb-4">Why Join Plus?</h2>
               <p className="text-slate-400 max-w-2xl mx-auto">Experience the ultimate convenience. We handle everything so you can focus on playing.</p>
            </div>
