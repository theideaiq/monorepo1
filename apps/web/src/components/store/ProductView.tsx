...
export function ProductView({ product }: ProductViewProps) {
  ...
  const handleAttributeChange = (key: string, value: string) => {
    ...
    const matchingVariant = product.variants.find(v => v.attributes[key] === value);
  ...
  const handleAddToCart = () => {
    ...
    if (hasVariants && Object.keys(attributes).length > Object.keys(selections).length) {
    ...
    const variantId = hasVariants ? `${product.id}-${Object.values(selections).join('-')}` : undefined;
           ...
           <div className="relative aspect-square bg-[#1a1a1a] rounded-3xl overflow-hidden border border-white/5">
              ...
              <div className="absolute top-4 left-4">
                 {product.condition === 'new' ? (
                    <span className="bg-brand-yellow text-brand-dark font-black px-3 py-1 rounded text-xs uppercase tracking-widest">New</span>
                 ) : (
                    <span className="bg-white/10 backdrop-blur text-white font-bold px-3 py-1 rounded text-xs uppercase tracking-widest border border-white/10">{product.condition}</span>
                 )}
              </div>
           </div>
        ...
        <div className="space-y-8">
           <div>
              ...
              <div className="flex items-center gap-4 text-sm text-slate-400">
                 ...
                 {product.isVerified && <CheckCircle2 size={16} className="text-blue-500" />}
              </div>
           </div>

           <div className="text-4xl font-black text-brand-yellow">
              {price} <span className="text-lg font-medium text-slate-500">IQD</span>
           </div>
