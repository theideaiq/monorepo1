  ...
  return (
    <div className="max-w-4xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8 pb-20">
      <div className="lg:col-span-2 space-y-4">
        ...
        {/* STEP 1: ADDRESS */}
        <div className={`rounded-3xl border transition-all overflow-hidden ${step === 1 ? 'bg-white/5 border-brand-yellow/50 shadow-[0_0_20px_rgba(250,204,21,0.1)]' : 'bg-black/40 border-white/5'}`}>
          <div
            className="p-6 flex items-center justify-between cursor-pointer"
            onClick={() => setStep(1)}
          >
             <div className="flex items-center gap-4">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold transition-colors ${step === 1 ? 'bg-brand-yellow text-brand-dark' : 'bg-white/10 text-slate-400'}`}>
                   {step > 1 ? <Check size={20} /> : '1'}
                </div>
                <h3 className={`text-xl font-bold ${step === 1 ? 'text-white' : 'text-slate-400'}`}>Shipping Address</h3>
             </div>
             {step > 1 && <button className="text-sm text-brand-yellow font-medium">Edit</button>}
          </div>
              ...
              >
                <div className="p-6 pt-0 border-t border-white/5">
                   <form onSubmit={handleAddressSubmit} className="space-y-4 mt-4">
                      ...
                      <div className="space-y-1">
                          <label className="text-xs text-slate-400">Address Details</label>
                          <textarea
                            required
                      ...
                      <div className="flex justify-end pt-4">
                         <Button type="submit" className="bg-brand-yellow text-brand-dark px-8 h-12 font-bold">
                            Continue to Payment
                         </Button>
                      </div>
                   </form>
        ...
        {/* STEP 2: PAYMENT */}
        <div className={`rounded-3xl border transition-all overflow-hidden ${step === 2 ? 'bg-white/5 border-brand-yellow/50 shadow-[0_0_20px_rgba(250,204,21,0.1)]' : 'bg-black/40 border-white/5'}`}>
          <div className="p-6 flex items-center gap-4">
             <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold transition-colors ${step === 2 ? 'bg-brand-yellow text-brand-dark' : 'bg-white/10 text-slate-400'}`}>
                {step < 2 ? <Lock size={18} /> : '2'}
             </div>
             <h3 className={`text-xl font-bold ${step === 2 ? 'text-white' : 'text-slate-400'}`}>Payment Method</h3>
          </div>
                   ...
                   <div className="p-6 pt-0 border-t border-white/5">
                      <div className="grid gap-4 mt-4">
                         ...
                         <label className="flex items-center gap-4 p-4 rounded-xl border border-brand-yellow bg-brand-yellow/10 cursor-pointer">
                            <input type="radio" name="payment" defaultChecked className="text-brand-yellow focus:ring-brand-yellow" />
                            <div className="flex-1">
                               <div className="font-bold text-white flex items-center gap-2">
                                  <CreditCard size={18} /> Pay via Wayl
                               </div>
                               <div className="text-xs text-slate-400">ZainCash, FIB, MasterCard, Visa</div>
                            </div>
                         </label>

                         <label className="flex items-center gap-4 p-4 rounded-xl border border-white/10 bg-black/20 cursor-pointer opacity-50">
                            <input type="radio" name="payment" disabled className="text-brand-yellow" />
                            <div className="flex-1">
                               <div className="font-bold text-slate-400">Cash on Delivery</div>
                               <div className="text-xs text-slate-500">Currently unavailable for high-value items</div>
                            </div>
                         </label>
                      </div>
               ...
               {items.map(item => (
                  <div key={item.id} className="flex gap-3">
                     <div className="w-12 h-12 bg-black rounded flex-shrink-0 relative overflow-hidden">
                        ...
                        <div className="absolute bottom-0 right-0 bg-brand-yellow text-brand-dark text-[10px] font-bold px-1 rounded-tl">{item.quantity}</div>
                     </div>
                     <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-brand-yellow font-bold">
                           {new Intl.NumberFormat('en-IQ').format(item.price * item.quantity)} IQD
                        </div>
                     </div>
                  </div>
               ))}
